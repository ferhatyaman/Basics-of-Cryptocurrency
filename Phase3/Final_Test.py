...
if BlockChainTestOn:
    ...
    if os.path.exists(BlockChainFileName) == True:
        ...
        m = "".join(blocks[0:LinkLen-1])
        m = m.encode()
        if PoW != hashlib.sha3_256(m).hexdigest():
            print ("Block chain does not validate:((",1)
            sys.exit()
        ...
        for i in range(1,int(blockCnt)):
            PrevHash = blocks[LinkLen*i-1]
            if(PrevHash != blocks[LinkLen*i]):
                print ("Block chain does not validate:((",2)
                sys.exit()
            PoW = blocks[(i+1)*LinkLen-1][:-1]
            m2 = "".join(blocks[i*LinkLen:(i+1)*LinkLen-1])
            m2 = m2.encode()
            if PoW != hashlib.sha3_256(m2).hexdigest():
                 print ("Block chain does not validate:((",3)
                 sys.exit()
            if PoW[0:PoWLen] != "0"*PoWLen:
                print ("Invalid proof of work:((")
                sys.exit()     
